...
class visa_central{
    ...
    public function getApplicationDetails($applicant, $user){
        ...
        if($db = new mysqli($server, $username, $password, $database)){
            $query = "SELECT `id`, `travel_agent`, `application_number`, `created`, `application_status` FROM `applications` WHERE `application_number` = ?";
            $connect = $db->prepare($query);
            $connect->bind_param("s", $applicant);
            $connect->execute();
            $count = $connect->store_result();
            $connect->bind_result($id, $agent, $app_num, $date, $status);
            $connect->fetch();
            $connect->close();
            $mag_shot_jpg= 'uploads/'.$app_num.'/mag-shot.jpg';
            $mag_shot_png= 'uploads/'.$app_num.'/mag-shot.png';
            $mag_shot_gif= 'uploads/'.$app_num.'/mag-shot.gif';
            $mag_shot_jpeg= 'uploads/'.$app_num.'/mag-shot.jpeg';
            $passport_jpg= 'uploads/'.$app_num.'/passport.jpg';
            $passport_png= 'uploads/'.$app_num.'/passport.png';
            $passport_gif= 'uploads/'.$app_num.'/passport.gif';
            $passport_jpeg= 'uploads/'.$app_num.'/passport.jpeg';
            $visa_jpg= 'uploads/'.$app_num.'/visa.jpg';
            $visa_png= 'uploads/'.$app_num.'/visa.png';
            $visa_gif= 'uploads/'.$app_num.'/visa.gif';
            $visa_jpeg= 'uploads/'.$app_num.'/visa.jpeg';
            if(!isset($_SESSION['applicant'])){
                $_SESSION['applicant'] =  $app_num;
            }else if(isset($_SESSION['applicant']) && $_SESSION['applicant'] != $app_num){
                $_SESSION['applicant'] =  $app_num;
            }
            echo 
            ...
            '<div class="thead">
                ...
                <div class="app_name">Application Number</div>
            ...
            echo 
                '<div class="tbody1">
                <div class="agent_alt">',$agent,'</div>',
                '<div class="app_name">',$app_num,'</div>',
    ...
    public function submitForm($agent, $app_no, $status, $pass_photo, $passport){
        ...
        if(!empty($agent) && !empty($status)){
            ...
            if($db = new mysqli($server, $username, $password, $database)){
                try{
                    $query = "INSERT INTO `applications` VALUES('',?,?,'null','null','null',000,null,?)";
                    $connect = $db->prepare($query);
                    $connect->bind_param("sss", $agent, $app_no, $status);                 
                    if($connect->execute()){
                        //$upload_fb = $this->upload_files($agent, $name);
                        return 'Application creation was successful, thank you.<br/>'. $this->upload_photo($agent, $app_no, $pass_photo).'<br/>'.$this->upload_passport($agent, $app_no, $passport);
